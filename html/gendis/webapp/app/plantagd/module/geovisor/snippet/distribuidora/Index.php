<?PHP
namespace App\Plantagd\Module\Geovisor\Snippet\Distribuidora;
...
class Index extends CoreResources {
    ...
    public function getData($item){
        ...
        $sql = "SELECT 
                g.distribuidor_id
                ,(select d.sigla from distribuidor as d where d.id = g.distribuidor_id) as distribuidor
                ,(select SUM(g.potencia_instalada) ) as potencia_instalada_total
                ,(select SUM(g2.potencia_instalada) from gd as g2  ".$wheregd." g2.distribuidor_id=g.distribuidor_id and g2.gd_categoria_id=1 ) as potencia_instalada_nanogeneracion
                ,(select SUM(g2.potencia_instalada) from gd as g2  ".$wheregd." g2.distribuidor_id=g.distribuidor_id and g2.gd_categoria_id=2 ) as potencia_instalada_microgeneracion
                ,(select SUM(g2.potencia_instalada) from gd as g2  ".$wheregd." g2.distribuidor_id=g.distribuidor_id and g2.gd_categoria_id=3 ) as potencia_instalada_minigeneracion
                ,(select COUNT(*) from gd as g2  ".$wheregd." g2.distribuidor_id=g.distribuidor_id and g2.gd_categoria_id=1 ) as total_nanogeneracion
                ,(select COUNT(*) from gd as g2  ".$wheregd." g2.distribuidor_id=g.distribuidor_id and g2.gd_categoria_id=2 ) as total_microgeneracion
                ,(select COUNT(*) from gd as g2  ".$wheregd." g2.distribuidor_id=g.distribuidor_id and g2.gd_categoria_id=3 ) as total_minigeneracion
        ...
        $sql .=" group by g.distribuidor_id";
        ...
        foreach ($info as $clave => $row){
            ...
            $info[$clave]["distribuidor"] = trim($row["distribuidor"]);

            $field = "potencia_instalada_nanogeneracion";
            $info[$clave][$field] = $row[$field]==""?0:$row[$field];
            $field = "potencia_instalada_microgeneracion";
            $info[$clave][$field] = $row[$field]==""?0:$row[$field];
            $field = "potencia_instalada_minigeneracion";
            $info[$clave][$field] = $row[$field]==""?0:$row[$field];

            $field = "total_nanogeneracion";
            $info[$clave][$field] = $row[$field]==""?0:$row[$field];
            $field = "total_microgeneracion";
            $info[$clave][$field] = $row[$field]==""?0:$row[$field];
            $field = "total_minigeneracion";
            $info[$clave][$field] = $row[$field]==""?0:$row[$field];
        }
